...
static void process_command(GtkEntry *entry, gpointer data)
{
    ...
    g_strlcpy(input, gtk_editable_get_text(GTK_EDITABLE(entry)), sizeof(input));
    cmd = trim(input);
    args = cmd;
    ...
    append_text("> ");
    append_text(gtk_editable_get_text(GTK_EDITABLE(entry)));
    ...
    else if(strcmp(cmd, "gemini") == 0)
    {
        if(args == NULL || trim(args)[0] == '\0') {
            ...
            char *py_cmd = g_strdup_printf("python gemini_api.py \"%s\"", args);
            char *stdout_buf = NULL;
            char *stderr_buf = NULL;
            int exit_status;
            
            append_text("Thinking...");
            // Force GTK to update UI here if possible, but standard g_spawn_command_line_sync will block
            if (g_spawn_command_line_sync(py_cmd, &stdout_buf, &stderr_buf, &exit_status, NULL)) {
                if (stdout_buf && stdout_buf[0] != '\0') {
                    append_text(stdout_buf);
                }
                if (stderr_buf && stderr_buf[0] != '\0') {
                    append_text(stderr_buf);
                }
            } else {
                append_text("Failed to execute python gemini_api.py.");
            }
...
static void on_terminal_gemini_prompt_run(GtkWidget *button, gpointer data) {
    ...
    if (prompt && *prompt) {
        ...
        if (g_spawn_command_line_sync(py_cmd, &stdout_buf, NULL, &exit_status, NULL) && stdout_buf && *stdout_buf) {
            ...
            gtk_text_buffer_insert(ai_chat_buffer, &iter, stdout_buf, -1);
            gtk_text_buffer_insert(ai_chat_buffer, &iter, "\n\n", -1);
        } else {
            gtk_text_buffer_get_end_iter(ai_chat_buffer, &iter);
